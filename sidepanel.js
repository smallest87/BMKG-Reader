// sidepanel.js - BMKG Reader (Fixed Order)
...
// --- 2. HELPER FUNCTIONS (Dipindah ke Atas agar aman) ---
...
function renderResults(data, filter = '') {
    ...
    data.hasil.forEach(g => {
        ...
        const filteredItems = g.data.filter(i => {
            const txt = (
                String(g.lokasi || '') + ' ' + 
                String(i.waktu || '') + ' ' + 
                String(i.kondisi || '') + ' ' + 
        ...
        const headerName = formatters.clean(g.lokasi);
        grp.innerHTML = `
            <div class="date-header">
                <span>${headerName}</span>
                <svg class="chevron" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5"/></svg>
            </div>
        `;
        ...
        filteredItems.forEach(i => {
            ...
            if (isDetail || (angin !== '-' && angin !== '')) {
                extra = `
                <div style="margin-top:6px; font-size:10px; color:#666; display:grid; grid-template-columns:1fr 1fr; gap:4px;">
                    <span style="background:#f1f5f9; padding:2px 6px; border-radius:3px;">Angin: ${angin}</span>
        ...
        const headerEl = grp.querySelector('.date-header');
        if(headerEl) headerEl.onclick = () => grp.classList.toggle('closed');
...
function renderChart(data) {
    ...
    // Safe parsing
    if (!data || !Array.isArray(data)) { hide('chartContainer'); return; }

    const ctx = cvs.getContext('2d');
    let pts=[], lbls=[]; chartPoints=[];
    
    data.forEach(g => {
        if(g.data) g.data.forEach(i => {
            let v=0;
            ...
            if(!isNaN(v)) { 
                const tgl = (g.lokasi || '').split(' ').slice(0,2).join(' ');
                const jam = (i.waktu || '').split(' ')[0];
                lbls.push({t:jam, f:`${i.waktu} (${tgl})`});
                pts.push(v);
            }
        });
    });
    ...
    const dpr=window.devicePixelRatio||1, rect=cvs.getBoundingClientRect();
...
if (chartCvs) {
    chartCvs.addEventListener('mousemove', (e) => {
        ...
        let hit=null, md=20;
        chartPoints.forEach(p=>{ const d=Math.sqrt((mx-p.x)**2+(my-p.y)**2); if(d<md){md=d;hit=p} });
        if(hit){ 
            tooltip.style.display='block'; tooltip.style.left=hit.x+'px'; tooltip.style.top=hit.y+'px'; 
            tooltip.innerHTML=`<strong>${hit.label}</strong><br>${hit.val} ${hit.unit}`; 
...
document.getElementById('scrapeBtn').addEventListener('click', async () => {
    ...
    chrome.tabs.sendMessage(tab.id, { action: "scrape_weather", mode: activeMode }, (response) => {
        if (chrome.runtime.lastError) {
            updateStatus("Error: Refresh halaman web BMKG.", true); return;
        }
        ...
        try {
            if (activeMode === 'detail') {
                ...
                show('chartContainer');
                setTimeout(() => {
                    try { renderChart(response.hasil); } 
                    catch(err) { console.error("Chart render error:", err); hide('chartContainer'); }
                }, 50);
            } else {
            ...
            console.error(e);
            updateStatus("Terjadi kesalahan tampilan.", true);
        }
    });
});